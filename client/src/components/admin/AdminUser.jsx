...
import {deleteUser,editUser, editUserbane} from "../../actions/actionAdmin"
        ...
        Swal.fire({
            ...
            confirmButtonText: 'Si'
                ...
                if (result.isConfirmed) {
                    const a=e.target.value.split(',')
                    const id=a[0];
                    const body={[e.target.name]: a[1]}
                    dispatch(editUserbane(id,body))
                    Swal.fire(
                    'Modificado',
            ...
            <td>
              <div className="btn-group d-flex justify-content-center" role="group" aria-label="Button group with nested dropdown">
                <div className="btn-group" role="group" >
                  ...
                  { e.banned ? 
                  <li>
                    <button className="dropdown-item" name="banned" value={[e.id,false]} onClick={(e)=>handelBanned(e)}> No </button>
                  </li> 
                  :
                  <li>
                    <button className="dropdown-item" name="banned" value={[e.id,true]} onClick={(e)=>handelBanned(e)}> Si </button>
                  </li>}
                  </ul>
                </div>
              </div>
